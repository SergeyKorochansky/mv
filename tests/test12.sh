...
function test_12() {
    create_env
    print_yeallow 'Running Test #12: Simple backup with custom suffix'
    FILE1='file1'
    FILE2='file2'
    FILE3='file3'
    BAKUP_SUFFIX='^^'
    ...
    if [ ${EXIT_CODE} -eq 0 ]; then
        EXIT_CODE=1
        ${PROGRAM} ${TEST_DIR}/${FILE1} ${TEST_DIR}/${FILE3} --backup=simple --suffix=${BAKUP_SUFFIX} &&
        [[ ! -f ${TEST_DIR}/${FILE1} ]] &&
        get_checksum ${TEST_DIR}/${FILE3} 4 &&
        get_checksum ${TEST_DIR}/${FILE3}${BAKUP_SUFFIX} 5 &&
        get_checksum ${TEST_DIR}/${FILE2} 7 &&
        ${PROGRAM} ${TEST_DIR}/${FILE2} ${TEST_DIR}/${FILE3} --backup=simple --suffix=${BAKUP_SUFFIX} &&
        [[ ! -f ${TEST_DIR}/${FILE1} ]] &&
        [[ ! -f ${TEST_DIR}/${FILE2} ]] &&
